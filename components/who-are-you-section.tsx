...
type UserType = {
  ...
  position: string
  size: string
  poster?: string  // optional: add a lightweight poster if you have one
}
...
const userTypes: UserType[] = [
  ...
  },
  {
    ...
    position: "top-8 left-[36rem]",
    size: "w-44 h-[480px]",
    poster: "/influencer.png"
  },
]

/* ----------------------------------------------
   LAZY VIDEO (ref-forwarding + in-view loading)
   ---------------------------------------------- */
...
const LazyVideo = forwardRef<HTMLVideoElement, LazyVideoProps>(function LazyVideo(
  ...
  useEffect(() => {
    ...
    // Attach src only when in view first time
    if (inView && !loaded) {
      try {
        v.src = src
        v.load() // start fetching now
        setLoaded(true)
      } catch {
        /* no-op */
      }
  ...
  useEffect(() => {
    ...
    if (inView) {
      v.play().catch(() => {
        /* autoplay can fail on some mobile policies */
      })
    } else {
      v.pause()
      if (resetWhenHidden) v.currentTime = 0
    }
  }, [inView, playWhenVisible, resetWhenHidden])
  ...
  return (
    <div ref={containerRef} className="w-full h-full">
      <video
        ...
        preload="none"
        poster={poster}
        style={{ pointerEvents: "none" }} // never capture clicks
      />
    </div>
  )
})
...
function VideoCard({ userType, index }: VideoCardProps) {
  ...
  const isCardInView = useInView(cardRef, { margin: "100px 0px" })

  // On touch devices, there's no hover, so play when visible.
  const isTouch = useMemo(() => {
    if (typeof window === "undefined" || !window.matchMedia) return false
    return window.matchMedia("(hover: none)").matches
  }, [])

  const shouldPlay = (isTouch && isCardInView) || (isHovered && isCardInView)
  ...
  return (
    ...
    >
      <div className="relative h-full rounded-[6rem] overflow-hidden group cursor-pointer neon-border will-change-transform">
        <LazyVideo
          src={userType.video}
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6">
          ...
          <motion.div
            ...
            className="flex flex-col items-center space-y-4"
          >
            <JoinNowLink href={userType.link} />

            <div className="p-2 bg-black/50 backdrop-blur-sm rounded-full">
              {shouldPlay ? <Pause className="w-4 h-4 text-white" /> : <Play className="w-4 h-4 text-white" />}
            </div>
          </motion.div>
        </div>
      </div>
...
/* ----------------------------------------------
   JOIN NOW LINK (prefetch + free main thread)
   ---------------------------------------------- */

function JoinNowLink({ href, className }: { href: string; className?: string }) {
  return (
    <Link
      href={href}
      prefetch
      onClick={() => {
        // Free decoder/main thread before route transition
      ...
      className={
        className ??
        "flex items-center space-x-2 bg-primary/80 backdrop-blur-sm px-4 py-2 rounded-full text-white font-medium hover:bg-primary transition-all duration-300 glow-effect text-sm"
      }
    >
      <span>Join Now</span>
      <ArrowRight className="w-4 h-4" />
...
export default function WhoAreYouSection() {
  ...
  // Proactively prefetch all register routes
  useEffect(() => {
    userTypes.forEach((u) => router.prefetch(u.link))
  }, [router])
            ...
            <div className="lg:hidden col-span-full grid grid-cols-2 gap-6 mt-12">
              {userTypes.map((userType, index) => (
                <motion.div
                  ...
                  className="relative h-64 rounded-[2rem] overflow-hidden neon-border"
                >
                  {/* On mobile, auto-play when visible (no hover) */}
                  <LazyVideo
                    src={userType.video}
                    className="w-full h-full object-cover"
